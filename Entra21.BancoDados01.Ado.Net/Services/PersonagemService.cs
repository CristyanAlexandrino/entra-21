...
namespace Entra21.BancoDados01.Ado.Net.Services
{
    internal class PersonagemService : IPersonagemService
    {
        public void Apagar(int id)
        {
            ...
            // Substituir os 0 no comando da query que será executada no banco de dados, prevenido SQL Injection
        ...
        public void Cadastrar(Personagem personagem)
        {
            var conexao = new Conexao().Conectar();
            var comando = conexao.CreateCommand();
            comando.CommandText = @"INSERT INTO personagens (id_tipo_personagem, id_editora, nome) 
...
INNER JOIN editoras AS e ON(p.id_editora = e.id)";
            ...
            // Executa SELECT armazenando na tabela em memória
        ...
        {
            // Connctado no banco de dados e defeinido a query que será executada
            var conexao = new Conexao().Conectar();
            var comando = conexao.CreateCommand();
            comando.CommandText =
                "UPDATE personagens SET id_tipo_personagem = @ID_TIPO_PERSONAGEM, id_editora = @ID_EDITORA, nome = @NOME WHERE id = @ID";

            // Substituir os 0 do update com os valores preenchidos pelo usuário
